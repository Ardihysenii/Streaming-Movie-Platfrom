                  ...
                  <div className="player-subtitle-menu" role="dialog" aria-label="Subtitle settings">
                    ...
                    {subtitleCustomizeOpen ? (
                      <div className="player-subtitle-customize">
                        ...
                        <label>Sync <span>{subtitleOffset > 0 ? "+" : ""}{subtitleOffset.toFixed(1)}s</span><input type="range" min="-5" max="5" step="0.5" value={subtitleOffset} style={{ "--range-progress": `${((subtitleOffset + 5) / 10) * 100}%` }} onChange={(event) => setSubtitleOffset(Number(event.target.value))} /></label>
                      </div>
                    ) : null}
