...
export function MovieCard({ movie, rank, progress, onRemove, priority = false, continueWatching = false, removeActionLabel = "Continue Watching" }: MovieCardProps) {
    ...
    // The preview scales the card slightly, so remeasure after that transform
    // lands to keep the portaled information surface attached to the same bounds.
    useEffect(() => {
          if (!hoverOpen) return;
    ...
    return (
          <article
                ...
                >
            {rankLabel ? <span className="rank-number">{rankLabel}</span>span> : null}
                <div className="movie-card-body">
                        <Link ref={posterRef} href={href} aria-label={"View " + movie.title} className="poster-link">
                          ...
                          {typeof progress === "number" ? <span className="watch-progress" aria-label={Math.round(progress) + " percent watched"}><i style={{ width: String(Math.min(100, Math.max(2, progress))) + "%" }} /></span>span> : null}
                        </Link>Link>
                  {hoverOpen && typeof document !== "undefined" ? createPortal(
                            <div
                                        ...
                                        >
                                        <strong>{movie.title}</strong>strong>
                                        <div className="card-hover-actions">
                                                      <div className="card-hover-actions-left">
                                                                      <Link className="card-hover-btn card-hover-btn--play" href={href} aria-label={"Play " + movie.title} title="Play"><PlayIcon /></Link>Link>
                                                                      <WishlistButton movie={movie} className="card-hover-btn--list" />
                                                      </div>div>
                                                      <Link className="card-hover-btn card-hover-btn--more" href={href} aria-label={"More info about " + movie.title} title="More info">•••</Link>Link>
                                        </div>div>
                                        <div className="card-hover-meta">
                                                      <span className="card-hover-match">{Math.round(Math.min(99, Math.max(72, movie.vote_average * 10)))}% Match</span>span>
                                                      <span className="card-hover-pill">{releaseYear(movie)}</span>span>
                                                      <span>{movie.media_type === "tv" ? "TV Show" : "Movie"}</span>span>
                                                      <span className="card-hover-rating"><StarIcon /> {movie.vote_average.toFixed(1)}</span>span>
                                        </div>div>
                              {movie.overview ? <p className="card-hover-overview">{movie.overview}</p>p> : null}
                            </div>div>,
                          document.body,
                          ) : null}
                        <div className="movie-card-copy">
                                  <Link href={href}>{movie.title}</Link>Link>
                                  <span className="movie-card-meta"><span className="movie-card-rating" aria-label={movie.vote_average.toFixed(1) + " rating out of 10"}><StarIcon /> {movie.vote_average.toFixed(1)}</span>span><span className="movie-card-year">{releaseYear(movie)}</span>span><span className="movie-card-type">{movie.media_type === "tv" ? "TV Show" : "Movie"}</span>span></span>span>
                        </div>div>
                  {onRemove ? <button className="remove-card" onClick={onRemove} aria-label={"Remove " + movie.title + " from " + removeActionLabel}><CloseIcon /></button>button> : null}
                </div>div>
          </article>article>
        );
}
